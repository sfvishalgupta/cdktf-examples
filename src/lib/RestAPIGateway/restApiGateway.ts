...
export class RestApiGateway extends Construct {
    ...
    private createProxyIntegration(name: string, obj: iProxyIntegration) {
        ...
        for (const method of obj.methods) {
            ...
            const proxyMethodRes: ApiGatewayMethodResponse = new ApiGatewayMethodResponse(this, name + '-proxy-method-response' + obj.name + "-" + method.name, {
                restApiId: this.apiGateway.id,
                resourceId: proxy.id,
                statusCode: "200",
                httpMethod: method.method ?? APIGateway_HTTPMethod.ANY,
            ...
            const apiInt: ApiGatewayIntegration = new ApiGatewayIntegration(this, name + '-proxy-integration' + obj.name + "-" + method.name, {
                httpMethod: proxyMethod.httpMethod,
                resourceId: proxyMethod.resourceId,
                restApiId: this.apiGateway.id,
                type: 'AWS_PROXY',
                integrationHttpMethod: APIGateway_HTTPMethod.POST,
                uri: this.getAPIIntegrationURI(method.lambdaName)
            })
            this.dependsOnResource.push(apiInt);

            if (method.schema) {
                const apiModel: ApiGatewayModel = new ApiGatewayModel(this, name + '-model-' + obj.name + "-" + method.name, {
                    restApiId: this.apiGateway.id,
            ...
            new LambdaPermission(this, name + '-apigateway-lambda-permission' + obj.name + "-" + method.name, {
